...
from .utils.model.LCCDE.LCCDE_test import LCCDE_test
from .utils.feature_extract.cicflowmeter.sniffer import cicflowmeter
...
def upload():
    filepath = app.config['UPLOAD_FOLDER']  # 外部函数赋值好的路径\pcaps
    upload = Upload()
    if request.method == 'GET':
        return render_template('./upload/upload.html')
    elif request.method == 'POST':
        bt_upload = request.values.get("upload")
        bt_capture = request.values.get("capture")
        global PCAP_NAME, PCAPS, TEST_PCAP
        PCAPS = None
        # 上传
        if bt_upload == 'upload':
            ...
            if upload.validate_on_submit():
                ...
                if allowed_file(pcapname):
                    ...
                    try:
                        TEST_PCAP = os.path.join(filepath, PCAP_NAME)
                        pcap.save(os.path.join(filepath, PCAP_NAME))
                        print(type(os.path.join(filepath, PCAP_NAME)))
                        print(str(os.path.join(filepath, PCAP_NAME)))
                        PCAPS = rdpcap(str(os.path.join(filepath, PCAP_NAME)))
                        # PCAPS = rdpcap("/root/Pcap-Analyzer/pcaps/zeyif90ntw.pcap")
                        print('恭喜你,上传成功！')
                        flash('恭喜你,上传成功！')
                        return render_template('./upload/upload.html')
            ...
            return render_template('./upload/upload.html')
        # 捕获
        if bt_capture == 'capture':
            capture()
            try:
                os.chdir("/root/Pcap-Analyzer/pcaps/")
                cur_dir = os.getcwd()  # 当前路径
                print(cur_dir)
                # pcap = rdpcap('capture.pcap')
                PCAPS = rdpcap('capture.pcap')
                TEST_PCAP = "/root/Pcap-Analyzer/pcaps/capture.pcap"
                # pcap.save(os.path.join(filepath, 'capture.pcap'))
                # print(type(os.path.join(filepath, 'capture.pcap')))
                # print(str(os.path.join(filepath, 'capture.pcap')))
                # PCAPS = rdpcap("/root/Pcap-Analyzer/pcaps/zeyif90ntw.pcap")
                print('恭喜你,捕获成功！')
                flash('恭喜你,捕获成功！')
                os.chdir("/root/Pcap-Analyzer/")
                return render_template('./upload/upload.html')
            except Exception as e:
                flash('捕获失败,错误信息:' +str(e))
                return render_template('./upload/upload.html')
            return render_template('./upload/upload.html')
        return render_template('./upload/upload.html')
...
@app.route('/rulematch/', methods=['POST', 'GET'])
def rulematch():
    if PCAPS == None:
        flash("请先上传要分析的数据包!")
        return redirect(url_for('upload'))
        ...
        if dataid:
            ...
            return render_template('./exceptions/rulematch.html', warning=warning_list)

# 智能检测


@app.route('/AIdetect/', methods=['POST', 'GET'])
def AIdetect():
    if PCAPS == None:
        flash("请先上传要分析的数据包!")
        return redirect(url_for('upload'))
        ...
        host_ip = get_host_ip(PCAPS)
        name1 = random_name()
        csv_path=app.config['CSV_FOLDER']+name1+'.csv'
        ...
        if request.method == 'POST':
            op_feature = request.form.get("feature_extractor")
            op_model = request.form.get("model")
            print(op_feature)
            print(op_model)

            if op_feature=="cicflowmeter":
                print("cicflowmeter")
                cicflowmeter(TEST_PCAP, csv_path)
            if op_model=="LCCDE":
                print("LCCDE")
                warning_list = LCCDE_test(csv_path, host_ip)
        ...
        if dataid:
            ...
            return render_template('./exceptions/AIdetect.html', warning=warning_list)
